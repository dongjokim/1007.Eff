...
TString FigDir;
TString OutTag;
void DrawEff(TString input, int DrawMode = 1 ){
    ...
    */
    ...
    if( VtxBin.size() == 0 ){
        VtxBin.push_back(-10);
        VtxBin.push_back(10);
        TrackCut.push_back("TPCOnly"); TrackCut.push_back("Raa"); TrackCut.push_back("TPC2A"); TrackCut.push_back("TPCplus"); TrackCut.push_back("Raa2011"); TrackCut.push_back("Hybrid2011A"); TrackCut.push_back("Hybrid2011B"); TrackCut.push_back("Hybrid2011"); TrackCut.push_back("H22011"); TrackCut.push_back("H32011");
    }
    ...
    if( DrawMode & 1 ){
        ...
        for( UInt_t ivtx=0;ivtx<VtxBin.size()-1;ivtx++ ){
            for( UInt_t icent=0;icent<CentBin.size()-1;icent++ ){
                mc(++iCan);
                hset0( Form("Con_cent%d",icent),lpt,hpt,0,0.5, "p_{T} [GeV/c]", "Contamination");
                leg0(0.7,0.7,0.9,0.9,Title);
                if(!rt.IsPP()) led->AddEntry(hfr, CentStr[icent], "p" );
                for( UInt_t icut=0;icut<TrackCut.size();icut++ ){
                    if( icut!=0 && icut!=1 && icut!=3 && icut!=7 ) continue;
...
void FinishMC(){
    ...
    gPad->GetCanvas()->SaveAs( TString(OutTag+"_"+Name+".eps").Data() );
}
